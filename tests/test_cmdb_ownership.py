...
def _artifact(
    path: Path, scan: str, scope: str, *, complete: bool = True, apply=False
) -> Path:
    data = {
...
def test_shadow_gate_requires_three_verified_same_scope_dry_runs(
    tmp_path: Path,
) -> None:
...
def test_shadow_gate_rejects_apply_incomplete_scope_drift_and_checksum(
    tmp_path: Path,
) -> None:
...
def test_backfill_requires_explicit_matching_scope_and_digest(tmp_path: Path) -> None:
    ...
    plan = plan_ownership_backfill(
        mgr, [OwnershipEntry(ci.id, "systemd:nor", scope)], gate
    )
...
def test_backfill_rejects_state_changed_after_review_before_any_write(
    tmp_path: Path,
) -> None:
    evidence = tmp_path / "evidence"
    evidence.mkdir()
