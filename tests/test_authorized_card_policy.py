...
def operator_authority(rig: Rig):
    import capauth
    ...
    if any(not hasattr(capauth, name) for name in required):
        pytest.skip("installed CapAuth does not include operator-session currentness")
    revisions = capauth.CurrentPolicyRevisions(
        issuer="1" * 64,
        principal="2" * 64,
    ...
    sessions = capauth.OperatorSessionManager(
        backend=capauth.InMemoryOperatorSessionBackendForTests(),
        current_revisions=lambda _binding: state["revisions"],
        enabled=True,
        clock=rig.clock,
    )
    ...
    class Issuer:
        def issue(self, request):
            assert type(request) is capauth.DelegatedCapabilityIssuance
            return rig.issuer.issue_root(
    ...
    factory = capauth.InProcessIssuerFactory(
        sessions=sessions,
        config=capauth.DashboardIssuerAuthorizationConfigV1(
            allowed_origin=ORIGIN,
            node_id=rig.binding.node_id,
            purpose=rig.binding.purpose,
            capability=rig.binding.capability,
            operation="read",
            target=rig.binding.target,
            resource_type=rig.binding.resource_type,
            resource_id=rig.binding.resource_id,
            owner_policy_revision=rig.binding.owner_policy_revision,
            ttl_seconds=60,
        ),
        issuer=Issuer(),
        enabled=True,
    ...
    result, verifier = factory.authorize(
        capauth.InProcessIssuerRequest(
            session_cookie=cookie,
            csrf_token=csrf,
            request_nonce="operator-request-1",
            device_fingerprint=device,
        ),
        rig.authorizer,
        rig.invocation(),
    ...
    assert result.allow and result.context is not None
    assert type(verifier) is capauth.OperatorSessionCurrentnessVerifier
    return result.context, verifier, sessions, cookie, state


def test_operator_session_currentness_allows_now_and_portfolio_snapshot() -> None:
    rig = Rig()
    context, verifier, _sessions, _cookie, _state = operator_authority(rig)
    ...
    assert result["records"][0]["record_id"] == "source"
    assert rig.store.fold_calls == ["source"]


@pytest.mark.parametrize("invalidate", ["revoke", "stale"])
def test_operator_session_revocation_or_staleness_denies_owner_read(invalidate) -> None:
    rig = Rig()
    context, verifier, sessions, cookie, state = operator_authority(rig)
    if invalidate == "revoke":
        sessions.revoke(cookie)
    else:
        state["revisions"] = state["revisions"].model_copy(update={"revocation": "9" * 64})
...
def test_operator_session_verifier_is_context_bound_and_single_use() -> None:
    rig = Rig()
    first_context, first, *_ = operator_authority(rig)
    second_context, second, *_ = operator_authority(rig)

    wrong_context = rig.provider.read(
        first_context,
        SCOPE,
        Path("/unused"),
        currentness_verifier=second,
        now=NOW,
    )
    current = rig.provider.read(
        first_context,
        SCOPE,
        Path("/unused"),
        currentness_verifier=first,
        now=NOW,
    )
    ...
    assert wrong_context["truth_state"] == "unknown"
    assert current["truth_state"] == "current"
