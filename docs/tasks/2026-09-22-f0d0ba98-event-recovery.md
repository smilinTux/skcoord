...
Do not inspect or modify live event JSONL, execute a live recovery, expose credentials, install, deploy, push, or alter provider state. Rollback is supported only for the exact shard bound in a valid receipt and is exercised only on synthetic fixtures by this task. Because pre-upgrade appenders lock the replaceable data inode instead of the new stable sidecar lock, live apply or rollback requires all writers on the target host to be upgraded or quiesced. The plan records this technical prerequisite and the supported CLI requires an explicit quiescence assertion for both mutations. Verification uses the isolated SKCoord source through `PYTHONPATH`; it does not install either candidate.
